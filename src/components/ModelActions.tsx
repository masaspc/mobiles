...
export default function ModelActions({id}:{id:string}) {
  ...
  const add=()=>{ const p=new URLSearchParams(location.search); const ids=(p.get('compare')??'').split(',').filter(Boolean); if(!ids.includes(id)&&ids.length<5)ids.push(id); location.href=withBase(`compare/?ids=${ids.join(',')}`); };
